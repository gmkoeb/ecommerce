...
class TestProductService:
    ...
    def test_list_products(self, test_db, products_service: ProductsService):
        ...
        company = companies_repository.create_company(corporate_name='Razer Unlimited', brand_name='Razer', 
                                                      ...
                                                      state='Test State', email='email@test.com', 
                                                      registration_number='123456789')
        products_repository.create_product(company_id=company.id, name="Gaming Keyboard", category="Gaming Keyboards", 
                                           price=1000, model="Razer Bla", 
                                           description="Basic Keyboard")
        products_repository.create_product(company_id=company.id, name="Keyboard", category="Keyboards", 
                                           price=2000, model="Razer Office", 
                                           description="Keyboard")
        

        products = products_service.list_categories()
        
        assert len(products) == 2
        assert products == ["Gaming Keyboards", "Keyboards"]
