...
export default function Cart() {
  ...
  useEffect(() => {
    ...
    cart.map(product =>
      setTotal(
        (prevTotal: number) => prevTotal + product.price * product.quantity
      )
    )
  }, [cart])
    ...
    <div className="px-2 border border-gray-400 rounded-md bg-white absolute mt-14 w-96 max-h-96 text-center right-4 text-black overflow-auto">
      ...
      {cart.length === 0 ? (
        ...
        <>
          {cart.map(product => (
            <div
              className="grid grid-cols-2 border-b mb-4 items-center"
              key={product.id}
            >
          ...
          <div className="flex justify-center items-center gap-4 mb-2">
            <p>Total: ${total}</p>
            <button
              className="hover: cursor-pointer font-bold flex items-center gap-2 border p-1 text-sm hover rounded hover:bg-red-500 duration-300"
              onClick={removeAll}
              type="button"
            >
              Empty Cart <Trash2 size={20} />
            </button>
          </div>
        </>
      )}
