    ...
    <div className="slider-container w-[70%]">
      <Slider {...settings}>
        {products.map((product) => (
          <div key={product.id}>
            <Link href={`/products/${product.id}`} className="flex flex-col items-center text-center font-bold">
              <img className="w-30 h-20" src={`/products/${product.name.split(" ").join("_").toLocaleLowerCase()}.png`} alt={`${product.name}`} />
              <p className="my-2">
                {product.name}
              </p>
              <p>
